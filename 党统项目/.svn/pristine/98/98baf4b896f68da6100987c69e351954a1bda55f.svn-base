    ...
    <div class="app-main" :style="{'width':'100%','height':'100%'}">
        <div class="layout  start-box box-orient" :style="{'width':'100%','height':'100%'}">
            ...
            <Row type="flex" class="box-flex">
                ...
                <i-col span="19" class="start-box box-orient" style="overflow: hidden;">
                    <!-- 筛选结构 -->
                    <div class="layout-breadcrumb"  @click="closeDefault">
                        <subNav ref="subnav" :remberRow="remberRow"></subNav>
                    </div>
                    <!-- 表单主结构 -->
                    <div class="layout-content box-flex" style="position:realtive;">
                        <div class="content-padding start-box box-orient"  @click="closeDefault">
                            <!-- 搜索和过滤 -->
    ...
    export default {
        ...
        methods: {
            ...
            CombinationQuery() {
              ...
              let qs = require('qs');
              let type='';
              if(!that.$route.params.queryId){
                return;
              }
              if (/^orgManage$/.test(this.$route.name)) {
                type='1';
              }else if(/^dangyuan$/.test(this.$route.name)){
                type='2';
              }else if(/^applicantManage$/.test(this.$route.name)){
                type='3';
              }else if(/^unitManage$/.test(this.$route.name)){
                type='4';
              }
              this.axios({
                method:'post',
